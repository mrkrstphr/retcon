...
import { deletePageFromArchive } from '@retcon/common/lib';
import { rm } from 'fs/promises';
import { join, normalize, resolve } from 'path';
import { data } from 'react-router';
import { protectRoute } from '~/lib/protectRoute';
import { sqidToIdOr404 } from '~/lib/sqids';
import type { Route } from './+types/DeletePage';
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  try {
    const { newPageCount } = await deletePageFromArchive(
      comic.fileName,
      comicId,
      pageNumber,
      dataDirectory,
    ...
    console.error(`Error deleting page ${pageNumber} of comic ${comicId}:`, error);
    return data(
      {
